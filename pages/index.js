...
export default function Home() {
  ...
  const [promoForm, setPromoForm] = useState({ code:'', type:'percent', value:'', start_date:'', end_date:'', max_uses:'' })
  const [promoError, setPromoError] = useState('')
  const [promoCode, setPromoCode] = useState('')
  const [promoDiscount, setPromoDiscount] = useState(null)
  const [promoApplied, setPromoApplied] = useState(null)
  const [bakeDayEditModal, setBakeDayEditModal] = useState(false)
  const [editingBakeDay, setEditingBakeDay] = useState(null)
  const [bakeDayForm, setBakeDayForm] = useState({})
  const [bakeDayNewModal, setBakeDayNewModal] = useState(false)
  const [newBakeDayForm, setNewBakeDayForm] = useState({ bake_date:'', label:'', pickup_start:'11:00', pickup_end:'15:00', pickup_instructions:'' })
  const [customRequests, setCustomRequests] = useState([])
  const [requestRespondModal, setRequestRespondModal] = useState(false)
  const [respondingRequest, setRespondingRequest] = useState(null)
  const [respondForm, setRespondForm] = useState({ status:'confirmed', message:'' })
  const [settingsForm, setSettingsForm] = useState({ bakery_name:'Elizabeth & Co.', pickup_city:'Upper Yucaipa, CA', pickup_address:'123 Willow Lane, Upper Yucaipa, CA 92399', zelle_recipient:'elizabethandco@gmail.com', venmo_handle:'@ElizabethandCo-Bread', tax_rate:'8.5', contact_phone:'(909) 555-0100' })
  ...
  const [editProductForm, setEditProductForm] = useState({ name:'', description:'', price:'', category:'sourdough', emoji:'🍞', variants:'', variant_prices:{}, is_active:true, image_url:'' })
  ...
  async function loadDashStats() {
    const result = await api('admin', { action:'get_stats' })
    if (result.stats) setDashStats(result.stats)
  }
  ...
  async function loadSettings() {
    const result = await api('admin', { action:'get_settings' })
    if (result.settings && Object.keys(result.settings).length > 0) {
      setSettingsForm(f => ({ ...f, ...result.settings }))
    }
  }

  async function saveSettings() {
    const result = await api('admin', { action:'save_settings', ...settingsForm })
  ...
  async function createPromo() {
    setPromoError('')
    if (!promoForm.code || !promoForm.value) { setPromoError('Code and value are required'); return }
    const result = await api('admin', { action:'create_promo', ...promoForm })
    if (result.error) { setPromoError(result.error); return }
    setPromoModal(false)
    setPromoForm({ code:'', type:'percent', value:'', start_date:'', end_date:'', max_uses:'' })
    loadPromoCodes()
  }

  async function togglePromo(id, is_active) {
    await api('admin', { action:'update_promo', id, is_active })
    loadPromoCodes()
  }

  async function deletePromo(id) {
    if (!confirm('Delete this promo code?')) return
    await api('admin', { action:'delete_promo', id })
    loadPromoCodes()
  }

  async function applyPromoCode() {
    if (!promoCode.trim()) return
    const result = await api('admin', { action:'validate_promo', code:promoCode, subtotal:cartSub() })
    if (result.error) { setPromoDiscount(null); setPromoApplied({ error: result.error }); return }
    setPromoApplied(result.promo)
    setPromoDiscount(result.discount)
  }

  function openEditBakeDay(bd) {
    setEditingBakeDay(bd)
    setBakeDayForm({ ...bd })
    setBakeDayEditModal(true)
  }

  async function saveBakeDay() {
    const result = await api('admin', { action:'update_bake_day', ...bakeDayForm })
    if (result.error) { alert(result.error); return }
    setBakeDayEditModal(false)
    loadBakeDays()
  }

  async function createBakeDay() {
    if (!newBakeDayForm.bake_date || !newBakeDayForm.label) { alert('Date and label are required'); return }
    const cutoff = new Date(newBakeDayForm.bake_date)
    cutoff.setDate(cutoff.getDate() - 2)
    const result = await api('admin', { action:'create_bake_day', ...newBakeDayForm, cutoff_datetime: cutoff.toISOString() })
    if (result.error) { alert(result.error); return }
    setBakeDayNewModal(false)
    setNewBakeDayForm({ bake_date:'', label:'', pickup_start:'11:00', pickup_end:'15:00', pickup_instructions:'' })
    loadBakeDays()
  }

  function openRespondRequest(req) {
    setRespondingRequest(req)
    const defaultMsg = req.status === 'confirmed'
      ? `Hi ${req.first_name}! Great news — your custom order request has been confirmed. Brynlee will be in touch with the details. Thank you for choosing Elizabeth & Co.! 🍞`
      : `Hi ${req.first_name}, thank you for reaching out to Elizabeth & Co. Unfortunately we're unable to accommodate your request at this time. We hope to see you at an upcoming bake day! 🍞`
    setRespondForm({ status:'confirmed', message: defaultMsg })
    setRequestRespondModal(true)
  }
  ...
  async function sendRequestResponse() {
    ...
    const result = await api('admin', {
      ...
      customerPhone: respondingRequest.phone,
      customerName: respondingRequest.first_name
    })
    if (result.error) { alert(result.error); return }
    setRequestRespondModal(false)
    loadCustomRequests()
    alert(`Response sent to ${respondingRequest.first_name} via text!`)
  }
  ...
  function openNewProduct() {
    ...
    setEditProductForm({ name:'', description:'', price:'', category:'sourdough', emoji:'🍞', variants:'', variant_prices:{}, is_active:true, image_url:'' })
    setEditProductError('')
    setEditProductModal(true)
  }
  ...
  async function saveProduct() {
    ...
    const payload = {
      ...
      variant_prices: editProductForm.variant_prices || {},
      image_url: editProductForm.image_url || null,
  ...
  return (
    <>
          ...
          <div className="admin-layout">
            <div className="admin-sidebar">
              {[{id:'dashboard',icon:'📊',label:'Dashboard'},{id:'products',icon:'🍞',label:'Products'},{id:'bakedays',icon:'📅',label:'Bake Days'},{id:'orders',icon:'📦',label:'Orders & Fulfillment'},{id:'requests',icon:'📬',label:'Custom Requests',badge:true},{id:'customers',icon:'👥',label:'Customers'},{id:'rewards',icon:'✦',label:'Rewards'},{id:'reports',icon:'📈',label:'Reports'},{id:'sms',icon:'📱',label:'SMS & Notifications'},{id:'security',icon:'🔒',label:'Admin Security'},{id:'settings',icon:'⚙️',label:'Settings'}].map(t=>(
                <button key={t.id} className={`admin-nav-item ${adminTab===t.id?'active':''}`} onClick={()=>{setAdminTab(t.id);if(t.id==='orders'&&adminBakeDay)loadAdminOrders(adminBakeDay.id);if(t.id==='customers')loadAdminCustomers();if(t.id==='requests')loadCustomRequests();if(t.id==='rewards')loadPromoCodes();if(t.id==='dashboard')loadDashStats();}}>
            ...
            <div className="admin-content">
              ...
              {adminTab==='rewards'&&(
                <>
                  <h2 className="admin-section-title">Rewards & Promos</h2>
                  ...
                  <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',margin:'1.5rem 0 1rem'}}>
                    <h3 style={{fontSize:18,fontWeight:500}}>Promo Codes</h3>
                    <button className="btn-primary" onClick={()=>setPromoModal(true)}>+ Create Promo Code</button>
                  </div>
                  <div className="admin-table">
                    ...
                    {promoCodes.map(p=>(
                      <div key={p.id} className="admin-table-row">
                        ...
                        <span className="col-act">
                          <button className="tbl-btn" onClick={()=>togglePromo(p.id,!p.is_active)}>{p.is_active?'Deactivate':'Activate'}</button>
                          <button className="tbl-btn red" onClick={()=>deletePromo(p.id)}>Delete</button>
                        </span>
                      </div>
                    ))}
                    {promoCodes.length===0&&<div style={{padding:'2rem',textAlign:'center',color:'var(--mid)',fontSize:13}}>No promo codes yet.</div>}
                  </div>
                  <div style={{marginTop:'2rem',fontSize:13,background:'var(--gold-light)',border:'1px solid rgba(196,151,58,0.2)',borderRadius:10,padding:'12px 14px',color:'#7A5A10'}}><strong>Earn:</strong> 1 point per $1 spent. &nbsp;<strong>Redeem:</strong> Preset discount blocks only — no partial redemptions.</div>
                  <div className="rewards-table" style={{marginTop:'1rem'}}>
              ...
              {adminTab==='bakedays'&&(
                <>
                  <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'1.5rem'}}>
                    ...
                    <button className="btn-primary" onClick={()=>setBakeDayNewModal(true)}>+ Schedule Bake Day</button>
                  </div>
                  <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(220px,1fr))',gap:14,marginBottom:'1.5rem'}}>
                    {bakeDays.map(bd=>(
                      <div key={bd.id} style={{background:'white',border:`1px solid ${bd.is_open?'var(--sage)':'var(--border)'}`,borderRadius:12,padding:'1.25rem'}}>
                        ...
                        <div style={{fontSize:13,color:'var(--mid)',marginTop:4}}>{bd.bake_date} · {bd.pickup_start} – {bd.pickup_end}</div>
                        <div style={{display:'flex',alignItems:'center',gap:8,marginTop:10}}>
                          <span className={`bake-status ${bd.is_open?'open':'soon'}`}>{bd.is_open?'Open':'Upcoming'}</span>
                          <button className="tbl-btn" style={{marginLeft:'auto'}} onClick={()=>openEditBakeDay(bd)}>Edit</button>
                        </div>
                      </div>
                    ))}
                  </div>
              ...
              {adminTab==='security'&&(
                <>
                  ...
                  <div style={{background:'var(--sage-light)',border:'1px solid rgba(107,124,92,0.2)',borderRadius:10,padding:'12px 14px',marginBottom:'1.5rem',fontSize:13,color:'var(--sage-dark)'}}>
                    <strong>Current login requires:</strong> Phone number + Password + 6-digit access code. All three must match to gain access.
                  </div>
                  <div style={{background:'white',border:'1px solid var(--border)',borderRadius:12,padding:'1.5rem',maxWidth:480,marginBottom:'1.5rem'}}>
                    ...
                    <div className="form-group"><label className="form-label">Current 6-digit access code (to verify)</label><input className="form-input" type="password" value={adminPwForm.currentCode} onChange={e=>setAdminPwForm(f=>({...f,currentCode:e.target.value}))} placeholder="••••••" maxLength={6} /></div>
                    ...
                    <div className="form-group"><label className="form-label">New password</label><input className="form-input" type="password" value={adminPwForm.newPassword||''} onChange={e=>setAdminPwForm(f=>({...f,newPassword:e.target.value}))} placeholder="New password" /></div>
                    <div className="form-group"><label className="form-label">New 6-digit access code</label><input className="form-input" type="password" value={adminPwForm.newCode} onChange={e=>setAdminPwForm(f=>({...f,newCode:e.target.value}))} placeholder="••••••" maxLength={6} /></div>
                    <p style={{fontSize:12,color:'var(--mid)',marginBottom:12}}>Leave any field blank to keep the current value.</p>
                    <button className="btn-primary" style={{width:'100%',padding:'12px'}} onClick={async()=>{
                      setAdminPwError('')
                      const result=await api('admin',{action:'update_admin_password',currentCode:adminPwForm.currentCode,newPhone:adminPwForm.newPhone||undefined,newCode:adminPwForm.newCode||undefined,newPassword:adminPwForm.newPassword||undefined})
                      if(result.error){setAdminPwError(result.error)}else{alert('Admin credentials updated! Note: phone/password changes take effect after next deployment.');setAdminPwForm({currentCode:'',newPhone:'',newCode:'',newPassword:''})}
              ...
              {adminTab==='settings'&&(
                <>
                  ...
                  {settingsSaved&&<div className="success-box" style={{maxWidth:480,marginBottom:'1rem'}}>✓ Settings saved successfully!</div>}
                  <div style={{background:'white',border:'1px solid var(--border)',borderRadius:12,padding:'1.5rem',maxWidth:480}}>
                    <div className="form-group"><label className="form-label">Bakery name</label><input className="form-input" value={settingsForm.bakery_name} onChange={e=>setSettingsForm(f=>({...f,bakery_name:e.target.value}))} /></div>
                    <div className="form-group"><label className="form-label">Pickup city (shown before ordering)</label><input className="form-input" value={settingsForm.pickup_city} onChange={e=>setSettingsForm(f=>({...f,pickup_city:e.target.value}))} /></div>
                    <div className="form-group"><label className="form-label">Full pickup address (confirmation only)</label><input className="form-input" value={settingsForm.pickup_address} onChange={e=>setSettingsForm(f=>({...f,pickup_address:e.target.value}))} /></div>
                    <div className="form-group"><label className="form-label">Contact phone</label><input className="form-input" value={settingsForm.contact_phone} onChange={e=>setSettingsForm(f=>({...f,contact_phone:e.target.value}))} /></div>
                    <div className="form-group"><label className="form-label">Zelle recipient</label><input className="form-input" value={settingsForm.zelle_recipient} onChange={e=>setSettingsForm(f=>({...f,zelle_recipient:e.target.value}))} /></div>
                    <div className="form-group"><label className="form-label">Venmo handle</label><input className="form-input" value={settingsForm.venmo_handle} onChange={e=>setSettingsForm(f=>({...f,venmo_handle:e.target.value}))} /></div>
                    <div className="form-group"><label className="form-label">Tax rate (%)</label><input className="form-input" type="number" step="0.1" value={settingsForm.tax_rate} onChange={e=>setSettingsForm(f=>({...f,tax_rate:e.target.value}))} /></div>
                    <button className="btn-primary" style={{marginTop:8,width:'100%',padding:'12px'}} onClick={saveSettings}>Save Settings</button>
                  </div>
                </>
              )}
              ...
              {adminTab==='requests'&&(
                <>
                  ...
                  <p style={{fontSize:13,color:'var(--mid)',marginBottom:'1.5rem'}}>Customer inquiries — respond via SMS. All responses are logged.</p>
                  {customRequests.length===0&&<div style={{padding:'2rem',textAlign:'center',color:'var(--mid)',fontSize:13,background:'white',borderRadius:12,border:'1px solid var(--border)'}}>No custom requests yet.</div>}
                  {customRequests.map((r)=>(
                    <div key={r.id} style={{background:'white',border:`1px solid ${r.status==='pending'?'var(--border)':r.status==='confirmed'?'var(--sage)':'#E0B0B0'}`,borderRadius:12,padding:'1.25rem',marginBottom:12}}>
                      <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:8}}>
                        <div><div style={{fontSize:15,fontWeight:500}}>{r.first_name} {r.last_name}</div><div style={{fontSize:12,color:'var(--mid)',marginTop:2}}>{r.phone}</div></div>
                        <div style={{display:'flex',gap:8,alignItems:'center'}}>
                          <span style={{fontSize:11,fontWeight:500,padding:'3px 10px',borderRadius:20,background:'#F8F1E9',color:'#6E5338'}}>{r.request_type?.replace('_',' ')}</span>
                          <span className={`status-pill ${r.status==='confirmed'?'paid':r.status==='declined'?'unfulfilled':'pending'}`}>{r.status}</span>
                        </div>
                      </div>
                      <div style={{fontSize:13,color:'var(--mid)',lineHeight:1.6,marginBottom:10}}>
                        <strong>Dates:</strong> {r.requested_dates}<br/>
                        <strong>Items:</strong> {r.items_requested}<br/>
                        {r.notes&&<><strong>Notes:</strong> {r.notes}</>}
                      </div>
                      {r.status==='pending'&&(
                        <div style={{display:'flex',gap:8}}>
                          <button style={{background:'var(--sage)',color:'white',border:'none',borderRadius:6,padding:'6px 14px',fontSize:12,fontWeight:500,cursor:'pointer'}} onClick={()=>{openRespondRequest(r);setRespondForm(f=>({...f,status:'confirmed',message:`Hi ${r.first_name}! Great news — your custom order request has been confirmed. Brynlee will be in touch with the details. Thank you for choosing Elizabeth & Co.! 🍞`}))}}>✓ Confirm & Text</button>
                          <button style={{background:'none',border:'1px solid #C0392B',color:'#C0392B',borderRadius:6,padding:'6px 14px',fontSize:12,cursor:'pointer'}} onClick={()=>{openRespondRequest(r);setRespondForm(f=>({...f,status:'declined',message:`Hi ${r.first_name}, thank you for reaching out to Elizabeth & Co. Unfortunately we're unable to accommodate your request at this time. We hope to see you at an upcoming bake day! 🍞`}))}}>✗ Decline & Text</button>
                        </div>
                      )}
                    </div>
                  ))}
        ...
        <div className="modal-bg">
          <div className="modal-wide">
            ...
            <div className="form-row">
              ...
              <div className="form-group"><label className="form-label">Emoji (fallback)</label><input className="form-input" value={editProductForm.emoji} onChange={e=>setEditProductForm(f=>({...f,emoji:e.target.value}))} /></div>
            </div>
            <div className="form-group">
              <label className="form-label">Product Photo</label>
              <div style={{display:'flex',gap:10,alignItems:'flex-start'}}>
                {editProductForm.image_url&&<img src={editProductForm.image_url} alt="preview" style={{width:60,height:60,objectFit:'cover',borderRadius:8,border:'1px solid var(--border)',flexShrink:0}} />}
                <div style={{flex:1}}>
                  <input type="file" accept="image/*" style={{display:'none'}} id="productImageUpload" onChange={async(e)=>{
                    const file=e.target.files?.[0]; if(!file) return
                    const url = await uploadProductImage(file)
                    if(url) setEditProductForm(f=>({...f,image_url:url}))
                  }} />
                  <label htmlFor="productImageUpload" style={{display:'block',background:'var(--cream)',border:'1.5px dashed var(--border-strong)',borderRadius:8,padding:'10px 14px',fontSize:13,color:'var(--mid)',cursor:'pointer',textAlign:'center'}}>
                    {imageUploading?'Uploading...':'📷 Click to upload photo'}
                  </label>
                  {editProductForm.image_url&&<div style={{fontSize:11,color:'var(--mid)',marginTop:4}}>Or paste URL: <input className="form-input" style={{marginTop:4,fontSize:12}} value={editProductForm.image_url} onChange={e=>setEditProductForm(f=>({...f,image_url:e.target.value}))} placeholder="https://..." /></div>}
                  {!editProductForm.image_url&&<input className="form-input" style={{marginTop:6,fontSize:12}} value={editProductForm.image_url} onChange={e=>setEditProductForm(f=>({...f,image_url:e.target.value}))} placeholder="Or paste image URL here..." />}
                </div>
              </div>
            </div>
            ...
            <div className="form-row">
              <div className="form-group"><label className="form-label">Base Price ($)</label><input className="form-input" type="number" step="0.01" value={editProductForm.price} onChange={e=>setEditProductForm(f=>({...f,price:e.target.value}))} /></div>
            ...
            <div className="form-group">
              <label className="form-label">Variants / Styles <span style={{fontWeight:400,color:'var(--mid)'}}>— comma-separated</span></label>
            ...
            {editProductForm.variants&&editProductForm.variants.trim()&&(
              <div className="form-group">
                <label className="form-label">Price per Variant <span style={{fontWeight:400,color:'var(--mid)'}}>— leave blank to use base price</span></label>
                {editProductForm.variants.split(',').map(v=>v.trim()).filter(Boolean).map(v=>(
                  <div key={v} style={{display:'flex',alignItems:'center',gap:10,marginBottom:8}}>
                    <span style={{fontSize:13,fontWeight:500,minWidth:140}}>{v}</span>
                    <span style={{fontSize:13,color:'var(--mid)'}}>$</span>
                    <input className="form-input" type="number" step="0.01" style={{maxWidth:100}} placeholder={editProductForm.price||'0.00'} value={editProductForm.variant_prices?.[v]||''} onChange={e=>setEditProductForm(f=>({...f,variant_prices:{...f.variant_prices,[v]:e.target.value}}))} />
                  </div>
                ))}
              </div>
            )}
            ...
            <div style={{display:'flex',gap:10}}>
              ...
              <button className="btn-primary" style={{flex:2,padding:'12px'}} onClick={saveProduct} disabled={editProductLoading||imageUploading}>{editProductLoading?'Saving...':imageUploading?'Uploading...':'Save Product'}</button>
            </div>
          </div>
        </div>
      )}
      ...
      {promoModal&&(
        <div className="modal-bg">
          <div className="modal">
            <button className="modal-close" onClick={()=>setPromoModal(false)}>✕</button>
            <p className="modal-title">Create Promo Code</p>
            {promoError&&<div className="error-box">{promoError}</div>}
            <div className="form-group"><label className="form-label">Code</label><input className="form-input" style={{textTransform:'uppercase'}} value={promoForm.code} onChange={e=>setPromoForm(f=>({...f,code:e.target.value.toUpperCase()}))} placeholder="e.g. SPRING20" /></div>
            <div className="form-row">
              <div className="form-group"><label className="form-label">Discount Type</label>
                <select className="form-input" value={promoForm.type} onChange={e=>setPromoForm(f=>({...f,type:e.target.value}))}>
                  <option value="percent">Percentage off (%)</option>
                  <option value="dollar">Dollar amount off ($)</option>
                </select>
              </div>
              <div className="form-group"><label className="form-label">Value</label><input className="form-input" type="number" step="0.01" value={promoForm.value} onChange={e=>setPromoForm(f=>({...f,value:e.target.value}))} placeholder={promoForm.type==='percent'?'20':'5.00'} /></div>
            </div>
            <div className="form-row">
              <div className="form-group"><label className="form-label">Start Date (optional)</label><input className="form-input" type="date" value={promoForm.start_date} onChange={e=>setPromoForm(f=>({...f,start_date:e.target.value}))} /></div>
              <div className="form-group"><label className="form-label">Expiry Date (optional)</label><input className="form-input" type="date" value={promoForm.end_date} onChange={e=>setPromoForm(f=>({...f,end_date:e.target.value}))} /></div>
            </div>
            <div className="form-group"><label className="form-label">Max Uses (optional — leave blank for unlimited)</label><input className="form-input" type="number" value={promoForm.max_uses} onChange={e=>setPromoForm(f=>({...f,max_uses:e.target.value}))} placeholder="e.g. 50" /></div>
            <div style={{background:'var(--cream)',borderRadius:8,padding:'10px 12px',fontSize:12,color:'var(--mid)',marginBottom:'1rem'}}>
              Once a code reaches its max uses, it automatically becomes inactive.
            </div>
            <div style={{display:'flex',gap:10}}>
              <button className="btn-ghost" style={{flex:1}} onClick={()=>setPromoModal(false)}>Cancel</button>
              <button className="btn-primary" style={{flex:2,padding:'12px'}} onClick={createPromo}>Create Promo Code</button>
            </div>
          </div>
        </div>
      )}
      ...
      {/* ══════════════════════════════════════════ BAKE DAY EDIT MODAL */}
      {bakeDayEditModal&&editingBakeDay&&(
        <div className="modal-bg">
          <div className="modal">
            <button className="modal-close" onClick={()=>setBakeDayEditModal(false)}>✕</button>
            <p className="modal-title">Edit Bake Day</p>
            <div className="form-group"><label className="form-label">Label</label><input className="form-input" value={bakeDayForm.label||''} onChange={e=>setBakeDayForm(f=>({...f,label:e.target.value}))} placeholder="e.g. Friday, April 4" /></div>
            <div className="form-group"><label className="form-label">Date</label><input className="form-input" type="date" value={bakeDayForm.bake_date||''} onChange={e=>setBakeDayForm(f=>({...f,bake_date:e.target.value}))} /></div>
            <div className="form-row">
              <div className="form-group"><label className="form-label">Pickup Start</label><input className="form-input" type="time" value={bakeDayForm.pickup_start||''} onChange={e=>setBakeDayForm(f=>({...f,pickup_start:e.target.value}))} /></div>
              <div className="form-group"><label className="form-label">Pickup End</label><input className="form-input" type="time" value={bakeDayForm.pickup_end||''} onChange={e=>setBakeDayForm(f=>({...f,pickup_end:e.target.value}))} /></div>
            </div>
            <div className="form-group"><label className="form-label">Pickup Instructions (shown in confirmation)</label><textarea className="form-input" rows={3} style={{height:'auto',resize:'none'}} value={bakeDayForm.pickup_instructions||''} onChange={e=>setBakeDayForm(f=>({...f,pickup_instructions:e.target.value}))} /></div>
            <div style={{display:'flex',alignItems:'center',gap:10,marginBottom:'1.25rem'}}>
              <input type="checkbox" checked={bakeDayForm.is_open||false} onChange={e=>setBakeDayForm(f=>({...f,is_open:e.target.checked}))} style={{width:15,height:15,accentColor:'var(--sage)'}} />
              <span style={{fontSize:13}}>Orders open (visible to customers)</span>
            </div>
            <div style={{display:'flex',gap:10}}>
              <button className="btn-ghost" style={{flex:1}} onClick={()=>setBakeDayEditModal(false)}>Cancel</button>
              <button className="btn-primary" style={{flex:2,padding:'12px'}} onClick={saveBakeDay}>Save Bake Day</button>
            </div>
          </div>
        </div>
      )}
      ...
      {bakeDayNewModal&&(
        <div className="modal-bg">
          <div className="modal">
            <button className="modal-close" onClick={()=>setBakeDayNewModal(false)}>✕</button>
            <p className="modal-title">Schedule Bake Day</p>
            <div className="form-group"><label className="form-label">Date</label><input className="form-input" type="date" value={newBakeDayForm.bake_date} onChange={e=>setNewBakeDayForm(f=>({...f,bake_date:e.target.value}))} /></div>
            <div className="form-group"><label className="form-label">Label</label><input className="form-input" value={newBakeDayForm.label} onChange={e=>setNewBakeDayForm(f=>({...f,label:e.target.value}))} placeholder="e.g. Friday, May 2" /></div>
            <div className="form-row">
              <div className="form-group"><label className="form-label">Pickup Start</label><input className="form-input" type="time" value={newBakeDayForm.pickup_start} onChange={e=>setNewBakeDayForm(f=>({...f,pickup_start:e.target.value}))} /></div>
              <div className="form-group"><label className="form-label">Pickup End</label><input className="form-input" type="time" value={newBakeDayForm.pickup_end} onChange={e=>setNewBakeDayForm(f=>({...f,pickup_end:e.target.value}))} /></div>
            </div>
            <div className="form-group"><label className="form-label">Pickup Instructions</label><textarea className="form-input" rows={2} style={{height:'auto',resize:'none'}} value={newBakeDayForm.pickup_instructions} onChange={e=>setNewBakeDayForm(f=>({...f,pickup_instructions:e.target.value}))} placeholder="Look for the signage at the front porch..." /></div>
            <p style={{fontSize:12,color:'var(--mid)',marginBottom:'1rem'}}>Order cutoff will be set to 48 hours before the bake date. Orders will be closed by default — open them when ready.</p>
            <div style={{display:'flex',gap:10}}>
              <button className="btn-ghost" style={{flex:1}} onClick={()=>setBakeDayNewModal(false)}>Cancel</button>
              <button className="btn-primary" style={{flex:2,padding:'12px'}} onClick={createBakeDay}>Create Bake Day</button>
            </div>
          </div>
        </div>
      )}

      {/* ══════════════════════════════════════════ REQUEST RESPOND MODAL */}
      {requestRespondModal&&respondingRequest&&(
        <div className="modal-bg">
          <div className="modal">
            <button className="modal-close" onClick={()=>setRequestRespondModal(false)}>✕</button>
            <p className="modal-title">{respondForm.status==='confirmed'?'✓ Confirm Request':'✗ Decline Request'}</p>
            <p className="modal-sub">Responding to {respondingRequest.first_name} {respondingRequest.last_name} · {respondingRequest.phone}</p>
            <div style={{background:'var(--cream)',borderRadius:8,padding:'10px 12px',fontSize:13,marginBottom:'1rem'}}>
              <strong>Request:</strong> {respondingRequest.requested_dates} — {respondingRequest.items_requested}
            </div>
            <div className="form-group">
              <label className="form-label">Response Type</label>
              <div style={{display:'flex',gap:8,marginBottom:'1rem'}}>
                <button className={`bake-sel-btn ${respondForm.status==='confirmed'?'active':''}`} onClick={()=>setRespondForm(f=>({...f,status:'confirmed',message:`Hi ${respondingRequest.first_name}! Great news — your custom order request has been confirmed. Brynlee will be in touch with the details. Thank you for choosing Elizabeth & Co.! 🍞`}))}>✓ Confirm</button>
                <button className={`bake-sel-btn ${respondForm.status==='declined'?'active':''}`} style={respondForm.status==='declined'?{borderColor:'#C0392B',background:'#FDECEA',color:'#C0392B'}:{}} onClick={()=>setRespondForm(f=>({...f,status:'declined',message:`Hi ${respondingRequest.first_name}, thank you for reaching out to Elizabeth & Co. Unfortunately we're unable to accommodate your request at this time. We hope to see you at an upcoming bake day! 🍞`}))}>✗ Decline</button>
              </div>
            </div>
            <div className="form-group">
              <label className="form-label">Message to Customer (sent via text)</label>
              <textarea className="form-input" rows={4} style={{height:'auto',resize:'none'}} value={respondForm.message} onChange={e=>setRespondForm(f=>({...f,message:e.target.value}))} />
            </div>
            <div style={{display:'flex',gap:10}}>
              <button className="btn-ghost" style={{flex:1}} onClick={()=>setRequestRespondModal(false)}>Cancel</button>
              <button className="btn-primary" style={{flex:2,padding:'12px',background:respondForm.status==='declined'?'#C0392B':undefined}} onClick={sendRequestResponse}>Send Response via Text</button>
            </div>
          </div>
        </div>
      )}
